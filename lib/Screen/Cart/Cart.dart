...
class StateCart extends State<Cart> with TickerProviderStateMixin {
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Row(
                                  ...
                                  children: [
                                    Text(
                                        getTranslated(context, 'Sub Total')!),
                                    Text(
                                      '₹ ${DesignConfiguration.getPriceFormat(context, context.read<CartProvider>().oriPrice)!} ',
                                      style: Theme.of(context)
                                          .textTheme
                                  ...
                                  Container(
                                      ...
                                      children: <Widget>[
                                        ...
                                        Padding(
                                          ...
                                          child: SimBtn(
                                            ...
                                            //     :
                                                () {
                                              log("VIJAY AMOUNT====${MIN_ALLOW_CART_AMT}");
                                              context
                                                  .read<CartProvider>()
                                                  .checkoutState!(
                                                    () {
                                    ...
                                    if (value != null &&
                                      ...
                                      confirmDialog();
                                              ...
                                              } else if (double.parse((MIN_ALLOW_CART_AMT?.isNotEmpty??false)?MIN_ALLOW_CART_AMT??"0":'0') > context.read<CartProvider>().oriPrice) {
                                                setSnackbar(
      ...
      try {
        ...
        apiBaseHelper.postAPICall(getAddressApi, parameter).then((getdata) {
          ...
          if (!error) {
            ...
            if (ISFLAT_DEL) {
              log("MINIMUM ==${MIN_AMT.runtimeType}");

              if ((context.read<CartProvider>().oriPrice) <
                  double.parse((MIN_AMT?.isNotEmpty??false)?MIN_AMT??"":'0')) {
                context.read<CartProvider>().deliveryCharge =
            ...
            if (ISFLAT_DEL) {
              log('MINIMUM AMOUNT===${MIN_AMT}');
              if ((context.read<CartProvider>().oriPrice) <
                  double.parse(
                      (MIN_AMT?.isNotEmpty ?? false) ? MIN_AMT ?? "" : '0')) {
                context.read<CartProvider>().deliveryCharge =
                    double.parse(CUR_DEL_CHR!);
              } else {
                context.read<CartProvider>().deliveryCharge = 0;
  ...
  Future<void> placeOrder(String? tranId) async {
    ...
    if (isNetworkAvail) {
      ...
      String? payVia;
      if (context.read<CartProvider>().payMethod ==
          getTranslated(context, 'Purchase on Credit')) {
        payVia = 'COD';
      } else if (context.read<CartProvider>().payMethod ==
          getTranslated(context, 'Purchase on credit - Onspot Payment (Discount)')) {
        payVia = 'COD';
      } else if (context.read<CartProvider>().payMethod ==
